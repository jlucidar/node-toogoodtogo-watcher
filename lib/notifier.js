...
function notifyIfChanged(businesses) {
  ...
  const slackMessage = createSlackMessage(filteredBusinesses);
...
function createSlackMessage(businesses) {
  return businesses
    .map(
      (business) =>
        `*HOY FELLOW RATZ !*

<https://share.toogoodtogo.com/item/${business.item.item_id}|🍽 ${business.display_name}>
💰 ${business.item.price_including_taxes.minor_units / 100}€
🥡 ${business.items_available} available
⏰ ${formatInterval(business)}`
    )
    .join("\n\n");
}
